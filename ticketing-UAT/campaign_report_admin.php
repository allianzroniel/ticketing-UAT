...
$campaignSql .= " GROUP BY t.campaign, t.concern_type, t.status ORDER BY t.campaign ASC, t.concern_type ASC, t.status ASC";
$res = $conn->prepare($campaignSql);
if ($isSiteScopedUser && $currentSite !== '') {
    $res->bind_param('s', $currentSite);
}
$res->execute();
$res = $res->get_result();
$initData = [];
